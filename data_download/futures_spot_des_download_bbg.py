...
for k in Ticker_dict.keys():
    ...
    ##########
    ###############
    # KLUGE INDEX FOR STIR IN PARTICULAR
    # if k == 'stirt':
    #     sector = 'Index'
    sector='Comdty'
    API_TASKS[k] = {
...
for k in [x for x in ['stirt']]: #API_TASKS.keys() if x not in ['curncy_spot','index_spot','stir_spot']]:  # API_TASKS.keys()]:
    ...
    fields = ["SHORT_NAME", "LONG_NAME", "EXCH_CODE", "FUT_EXCH_NAME_LONG",
              ...
              'ID_EXCH_SYMBOL', 'FUT_EXCH_NAME_LONG', 'ID_FULL_EXCHANGE_SYMBOL', 'ID_MIC_PRIM_EXCH',
              'QUOTE_UNITS','FUT_TRADING_UNITS','PRICE_MULTIPLIER','ID_BB_GLOBAL'
    ...
    for i in instruments:
        ...
        extended_flds = ['NAME', 'SHORT_NAME', 'FUT_INIT_SPEC_ML', 'FUT_SEC_SPEC_ML', 'UNDL_SPOT_TICKER', 'TIME_ZONE_NUM',
                         'TRADING_DAY_START_TIME_EOD', 'TRADING_DAY_END_TIME_EOD',
                         'DERIVATIVE_DELIVERY_TYPE', 'NOTIONAL_CURRENCY_1','ID_BB_GLOBAL'

                         ]  #,   'EXCHANGE_TRADING_SESSION_HOURS']  # messes it up,'FUT_TRADING_HRS']
        if not df_desc[k][i].loc[:,'fut_cur_gen_ticker'].isna().iloc[0]:
            '''
            We can link to the non-generic future info
            '''
            ...
            extended_fields = pd.DataFrame(index=df_desc[k][i].index,
                                           data=np.ones((len(df_desc[k][i].index),len(extended_flds))) * np.nan,
                                           columns = [x.lower() for x in extended_flds])


        # make sure missing fields get populated so we can merge later on
        for x in [y.lower() for y in extended_flds]:
            if x not in extended_fields.columns:
                extended_fields.loc[:, x] = np.nan


        rename_dict = {x.lower():'non_gen_'+x.lower() for x in extended_flds}
        extended_fields = extended_fields.rename(columns=rename_dict)

        df_desc[k][i] = pd.concat([df_desc[k][i], extended_fields], axis=1)



        spot_flds = ['NAME','SHORT_NAME','SECURITY_NAME','TRADING_DAY_START_TIME_EOD','TRADING_DAY_END_TIME_EOD',
                     'INDX_SOURCE', 'CALENDAR_CODE',
                     'ID_BB_GLOBAL']

        possible_ticker =df_desc[k][i].loc[i,'non_gen_undl_spot_ticker']
        if isinstance(possible_ticker,float):  #oops we f'ed it up
            possible_ticker = '' # make a string again
        if possible_ticker != '' :  #'.isna().iloc[0]:
            # undl_spot_ticker is populated!

            spot_ticker = df_desc[k][i].loc[:, 'non_gen_undl_spot_ticker'].iloc[0]
            bloomberg_sector = i.split(' ')[1]
            # if (bloomberg_sector == 'Curncy') or (bloomberg_sector=='CURNCY'):
            #     bloomberg_sector = 'Index'
            bloomberg_sector = 'Index'
            spot_ticker = spot_ticker + ' ' + bloomberg_sector
            # non_gen_fields = ''
            index_fields = blp.bdp(
                tickers=spot_ticker,
                flds=spot_flds
            )

            index_fields.index = df_desc[k][i].index

        else:
            index_fields = pd.DataFrame(index=df_desc[k][i].index,
                                           data=np.ones((len(df_desc[k][i].index), len(spot_flds))) * np.nan,
                                           columns=[x.lower() for x in spot_flds])

        for x in [y.lower() for y in spot_flds]:
            if x not in index_fields.columns:
                index_fields.loc[:, x] = np.nan


        rename_dict = {x.lower():'spot_'+x.lower() for x in spot_flds}
        index_fields = index_fields.rename(columns=rename_dict)

        df_desc[k][i] = pd.concat([df_desc[k][i], index_fields], axis=1)


        if df_desc[k][i].empty or df_desc[k][i].shape == (0,0):
            # ie BBG returned something even though it doesn't exist
            print('Popped one')  # shouldn't trigger now that there has been a change of logic
            _ = df_desc[k].pop(i)
            continue
    ...
    all_data = pd.concat(df_desc[k], axis=0)
    all_data = all_data.sort_index()
    all_data.index.name='generic_futures'
    all_data.to_csv('~/Dropbox/CCA/futures_data_raw/sec_desc_' + k + '_.csv')
    print('wrote ' '~/Dropbox/CCA/futures_data_raw/' + k + '_.csv')
    # total = pd.DataFrame()
    # for i in df_desc[k].keys():
    #
    #     print(i)
    #     total = pd.concat([total, df_desc[k][i].T], axis=1)
...
for kk in df_desc.keys():
    # pd.concat(df_desc[kk], axis=0).to_csv('~/Dropbox/CCA/futures_data_raw/again'+ kk +'_.csv')
...
#############
# total.to_csv('~/Dropbox/CCA/futures_data_raw/combo_data_futures_descriptions5.csv')
print('finfin')
# all_data.shape
