...
def monthlytable(nav) -> pd.DataFrame:
    ...
    # Compute all the intramonth-returns, instead of reapplying some monthly resampling of the Cumulative_Return
...
def build_table(cov_models, pnl_results):
    ...
    for m in pnl_results:
        ...
        perf = perf.loc[["Annual Sharpe Ratio (r_f = 0)","Annual Return", "Annua lVolatility",  "Max Drawdown", "Return", "Kurtosis"]]
        perf = perf.applymap(lambda x: float(x))
        full_table.loc[m, 'SR']  = round(perf['Cumulative_Return']['Annual Sharpe Ratio (r_f = 0)'], 2)
        full_table.loc[m, 'AnnRet']  = round(perf['Cumulative_Return']['Annual Return'], 2)
        full_table.loc[m, 'Vol']  = round(perf['Cumulative_Return']['Annual Volatility'], 2)
        full_table.loc[m, 'MaxDD']  = round(perf['Cumulative_Return']['Max Drawdown'], 2)
        full_table.loc[m, 'Return']  = round(perf['Cumulative_Return']['Return'], 2)
        full_table.loc[m, 'Kurtosis']  = round(perf['Cumulative_Return']['Kurtosis'], 2)
    return full_table
...
#     d["IR"] =  d["AV"] /  d["SD"]
#     d["SR_gross"] = sharpe_ratio(ptf_gross)
    d["SR"] = sharpe_ratio(ptf_ret)
    d["Calmar"] = calmar_ratio(ptf_ret)
    d["MDD"] = max_drawdown(ptf_ret)
    d["AnnRet"] = annual_return(ptf_ret) * 100
    d["Vol"] = annual_volatility(ptf_ret) * 100
...
#     ptf_ret_net = ptf_ret['Profit'].iloc[1:] - c * w.diff().abs().iloc[1:].sum(axis=1) 
    ...
    d["Turnover"] = w.diff().abs().iloc[1:].sum(axis=1).mean()
...
def build_perf_anal_table(cov_models, ptf_ret):
    ...
    for m in ptf_ret:
        ...
        perf = perf.loc[["SR",
                         ...
                         "Kurtosis" ]]
        perf = perf.applymap(lambda x: float(x))
        full_table.loc[m, "SR"]  = round(perf["Profit"]["SR"], 2)
        ...
        full_table.loc[m, "AnnRet"]  = round(perf["Profit"]["AnnRet"], 2)
        full_table.loc[m, "Vol"]  = round(perf["Profit"]["Vol"], 2)
...
def build_pos_anal_table(cov_models, w):
    
    order_of_models = cov_models
    full_table = pd.DataFrame(index=order_of_models)
    
    for m in w:
        perf = weight_summary(w[m])
        perf = perf.loc[["Turnover", "GL", "PL",  "herf",  "pos"]]
#         perf = perf.applymap(lambda x: float(x))
        full_table.loc[m, "Turnover"]  = round(perf["Turnover"], 2)
    ...
    pnl_fig = go.Figure(
        data=[
            go.Scatter(
                ...
                y=pnl_results[m]['Cumulative_Return'],
                name=m,
                opacity=0.8,
