...
def prepare_data(df):
    ...
    feature_cols = [
        'mean_reversion_signal1', 'mean_reversion_signal2', 'GKVol',
        'bid_ask_spread'
    ]
...
def ewrls_forecast(df, span=252, regularization=0.1):
    ...
    model = EWRLSRidge(num_features=len(feature_cols),
                       span=span,
...
def run_both_models(df,
    ...
    """
    # Run both models
    rolling_forecasts = rolling_regression_forecast(df,
                                                    window_size=rolling_window)
    ewrls_forecasts = ewrls_forecast(df,
                                     span=ewrls_span,
                                     regularization=regularization)

    # Combine results
    results = pd.concat([rolling_forecasts, ewrls_forecasts], axis=1)
